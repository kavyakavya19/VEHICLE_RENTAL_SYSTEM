...
function BookingContent() {
  ...
  useEffect(() => {
    // Try to get vehicle from sessionStorage (set by VehicleDetail on navigate)
  ...
  const handleCheckAvailability = async (e) => {
    ...
    try {
      const res = await API.post('bookings/check-availability/', {
        vehicle_id: parseInt(vehicleId, 10), start_date: startDate, end_date: endDate,
      });
      if (res.data.available === true) {
        ...
        const bookingDetails = {
          ...
          number_of_days: res.data.days, price_per_day: res.data.price_per_day,
  ...
  return (
    <div className="page-container animate-fade-in" style={{ paddingTop: '120px' }}>
      <div style={{ maxWidth: '600px', margin: '0 auto', background: 'rgba(255,255,255,0.05)', padding: '40px', borderRadius: '16px' }}>
        ...
        <p style={{ color: '#A1A1AA', marginBottom: '30px' }}>Select your trip dates to check vehicle availability.</p>
        {activeBooking && (
        ...
        <form onSubmit={handleCheckAvailability} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <div>
            <label style={{ display: 'block', marginBottom: '8px', color: '#A1A1AA' }}>Start Date</label>
            <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} min={new Date().toISOString().split('T')[0]} style={{ padding: '12px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.5)', color: 'white', width: '100%' }} required />
          </div>
          <div>
            <label style={{ display: 'block', marginBottom: '8px', color: '#A1A1AA' }}>End Date</label>
            <input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} min={startDate || new Date().toISOString().split('T')[0]} style={{ padding: '12px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.5)', color: 'white', width: '100%' }} required />
          </div>
